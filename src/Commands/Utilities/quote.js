...
module.exports = {
  ...
  usage: `q <Your line>`,
  react: "🍁",
  start: async (Akeno, m, { text, prefix, args, pushName }) => {
    if (!args[0] && !m.quoted)
      return m.reply(`Please provide a text (Type or mention a message) !`)
      
        if (m.quoted){
          try {
            userPfp = await Akeno.profilePictureUrl(m.quoted.sender, "image");
          } catch (e) {
            userPfp = botImage3;
          }
        }
    ...
    await Akeno.sendMessage(m.from, {sticker:stickerBuffer2}, { quoted: m }).then((result) => {
